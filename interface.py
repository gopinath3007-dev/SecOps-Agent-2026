...
def scrape_url(url):
    ...
    try:
        # If it's a standard GitHub link, try to point user to RAW for better quality
        ...
        # If it's a raw file (like code), return text directly
        if "raw.githubusercontent" in url or url.endswith(('.txt', '.yaral', '.yaml')):
            return res.text
            
        # If it's a webpage (Google Doc), scrape HTML
...
def ingest_knowledge(urls):
    ...
    # Split text into small pieces so Gemini can find specific rules/syntax
    text_splitter = RecursiveCharacterTextSplitter(chunk_size=1200, chunk_overlap=200)
    chunks = text_splitter.split_text(all_text)
    
    # Create the Librarian (Embeddings) and the Library (FAISS)
    # Using the 2026 standard langchain-huggingface integration
...
with st.sidebar:
    ...
    if st.session_state.vector_store:
        st.write("✅ Agent is currently Grounded in your URLs.")
    else:
...
with col2:
    ...
    if generate_btn:
        ...
        REQUIRED ACTIONS:
        1. If creating/fixing: Ensure strict YARA-L 2.0 syntax.
        2. If optimizing: Suggest specific threshold changes (e.g., event counts, time windows).
        3. Cross-reference the provided context. If the context contradicts the user, follow the context (Documentation is the Law).
        4. Provide the final YARA-L code block.
        """
